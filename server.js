...
const PORT = 4000 //listen on port 4000

app.use(cors()) //to avoid issues with cors
...
app.get('/', (request, response) => {
    response.sendFile(__dirname + '/index.html') //send the root file
})

app.get('/api/:alienName', (request, response) => {
    const aliensName = request.params.alienName.toLowerCase()
    if(aliens[aliensName]){ //checkif aliensName is in obj aliens
